...
export default function Dashboard({ setAuthToken, authToken, handleLogout }) {
  ...
  const fetchJournals = async () => {
    ...
    const response = await axios.post("https://ricksarkar.getskybuy.shop/api/all_journals",
       {
        Token: dbtoken
      }
    )
  ...
  const fetchUserDetails = async () => {
    try {
      ...
      const authResponse =  await axios.post('https://ricksarkar.getskybuy.shop/api/auth', {
        Email: details.email,
        });
